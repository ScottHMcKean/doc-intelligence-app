"""Processing service for Databricks serverless jobs."""
...
class ProcessingService:
    ...
    def _queue_existing_job(self, input_path: str, output_path: str, doc_hash: str) -> Tuple[bool, Optional[int], str]:
        ...
        try:
            ...
            return True, run_id, f"Processing job queued using existing job. Run ID: {run_id}"

        except Exception as e:
            logger.error(f"Failed to queue existing job: {str(e)}")
            return False, None, f"Failed to queue existing job: {str(e)}"

    def check_job_status(self, run_id: int) -> Tuple[bool, Optional[Dict[str, Any]], str]:
        """

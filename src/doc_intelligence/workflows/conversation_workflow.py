...
from ..services import (
    AuthService, DatabaseService, EmbeddingService, ChatService
)
...
class ConversationWorkflow:
    ...
    def send_message(
        ...
        try:
            ...
            response_success, ai_response, response_metadata = self.chat_service.generate_response(
                messages=messages,
    ...
    def get_user_conversations(self, username: Optional[str] = None) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            for conv in conversations:
                conv_list.append({
                    ...
                    "document_count": len(conv.document_ids) if conv.document_ids else 0,
    ...
    def add_documents_to_conversation(
        ...
        try:
            ...
            logger.info(f"Adding documents {document_hashes} to conversation {conversation_id}")
    ...
    def _retrieve_document_context(
        ...
        try:
            ...
            search_success, results, search_message = self.embedding_service.similarity_search(
                query=query,
                limit=limit,
    ...
    def _update_conversation_title_if_needed(
        ...
        try:
            ...
            summary_success, summary, suggested_title = self.chat_service.summarize_conversation(messages)
            
            if summary_success and suggested_title and suggested_title != "New Conversation":
                self.database_service.update_conversation_title(conversation_id, suggested_title)
